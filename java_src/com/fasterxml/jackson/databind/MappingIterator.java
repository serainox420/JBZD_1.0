...
public class MappingIterator<T> implements Closeable, Iterator<T> {
    ...
    public MappingIterator(JavaType javaType, JsonParser jsonParser, DeserializationContext deserializationContext, JsonDeserializer<?> jsonDeserializer, boolean z, Object obj) {
        ...
        JsonStreamContext mo184getParsingContext = jsonParser.mo184getParsingContext();
        if (z && jsonParser.isExpectedStartArrayToken()) {
            jsonParser.clearCurrentToken();
        } else {
            JsonToken currentToken = jsonParser.getCurrentToken();
            if (currentToken == JsonToken.START_OBJECT || currentToken == JsonToken.START_ARRAY) {
                mo184getParsingContext = mo184getParsingContext.mo404getParent();
            }
        }
        this._seqContext = mo184getParsingContext;
        this._state = 2;
    }
    ...
    public T nextValue() throws IOException {
        ...
        try {
            if (this._updatedValue == null) {
                t = this._deserializer.mo573deserialize(this._parser, this._context);
            } else {
    ...
    protected void _resync() throws IOException {
        ...
        if (jsonParser.mo184getParsingContext() == this._seqContext) {
            return;
        }
        while (true) {
